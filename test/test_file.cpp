/**
 * @file   test_file.cpp
 * @date   16.11.2015
 * @author Klemens
 *
 * Published under [Apache License 2.0](http://www.apache.org/licenses/LICENSE-2.0.html)
 */


#define BOOST_TEST_MODULE test_file
#include <boost/test/minimal.hpp>

#include <mw/test/parser/file.hpp>
#include <mw/test/parser/comment.hpp>

namespace x3 = boost::spirit::x3;


int test_main (int, char**)
{
	std::string s;

	auto beg = s.begin();
	auto itr = s.begin();
	auto end = s.begin();

	bool res;
	using mw::test::parser::file;
	using mw::test::parser::filename;
	using mw::test::parser::tests_file;
	using mw::test::parser::include;
	using mw::test::parser::skip;

	std::string parsed;

	auto p = [&](auto rule)
		{
			parsed.clear();
			beg = s.begin();
			itr = s.begin();
			end = s.end();
			res = x3::phrase_parse(itr, end, rule, skip, parsed);
		};

	s = "adings$-_z.hpp";
	p(filename);

	BOOST_CHECK(res);
	BOOST_CHECK(itr == end);
	BOOST_CHECK(parsed == "adings$-_z.hpp");


	s = "_ ";
	p(filename);

	BOOST_CHECK(res);
	BOOST_CHECK(itr == end);
	BOOST_CHECK(parsed == "_");

	s = "_: ";
	p(filename);

	BOOST_CHECK(res);
	BOOST_CHECK(itr == (beg +1 ));
	BOOST_CHECK(parsed == "_");

	s = "thingy.hpp";
	p(filename);

	BOOST_CHECK(res);
	BOOST_CHECK(itr == end);
	BOOST_CHECK(parsed == "thingy.hpp");
/*

	s = "file thingy.hpp ;";
	p(file);

	BOOST_CHECK(res);
	BOOST_CHECK(itr == (beg +1 ));
	BOOST_CHECK(parsed == "thingy.hpp");

	std::cerr << "'" << parsed << "'" <<std::endl;
*/


	return 0;
}
